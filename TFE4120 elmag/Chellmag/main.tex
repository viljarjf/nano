...
\newcommand{\E}{\ifmmode\vec{E}\else{$\vec{E}$ }\fi}
\newcommand{\e}{\ifmmode\epsilon_0\else{$\epsilon_0$ }\fi}
\newcommand{\p}{\ifmmode\vec{\underline{p}}\else{$\vec{\underline{p}}$ }\fi}
\newcommand{\D}{\ifmmode\vec{D}\else{$\vec{D}$ }\fi}
\renewcommand{\j}{\ifmmode\vec{j}\else{$\vec{j}$ }\fi}
\newcommand{\Epsilon}{\emf}
\newcommand{\B}{\ifmmode\vec{B}\else{$\vec{B}$ }\fi}
...
\subsection{Current}
    ...
    \end{equation}
    where the surface $d\Sigma$ goes towards zero, and $\j$ is the current density vector. 
...
\section{Magnetostatics}
    \subsection{Magnetic field}
        ...
        This holds in general for the orthogonal component of the velocity, whereas the parallel component is unaffected by the presence of the magnetic field. Therefore, if the velocity is neither parallel nor orthogonal to the magnetic field, the resulting motion is a spiral with constant velocity, pitch and radius.
